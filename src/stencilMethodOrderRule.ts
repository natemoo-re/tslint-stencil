...
export class Rule extends Lint.Rules.AbstractRule {
  ...
  public static FAILURE_STRING_ORDER =
    'Stencil methods should be in the following order: "%s"';
  ...
  found.map(methods => {
    ...
    return methods.map(({ node }) =>
      ctx.addFailureAtNode(
        node,
        Rule.FAILURE_STRING_ORDER.replace("%s", order.join(", "))
      )
    );
  });
}
